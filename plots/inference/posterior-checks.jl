...
pst = deserialize(projectdir("output/chains/population-atn/pst-samples-truncated-normal-4x1000.jls"));
...
pst = deserialize(projectdir("output/chains/population-atn/pst-samples-truncated-normal-fixed-4x1000.jls"));
...
begin    
    ...
    for sub in 1:22
    p = [meanpst[Symbol("α_a[$sub]"), :mean], meanpst[Symbol("ρ_t[$sub]"), :mean], 
    meanpst[Symbol("α_t[$sub]"), :mean],  3.432092090988647, 
    meanpst[Symbol("η[$sub]"), :mean]]
    pstprob = ODEProblem(atn_model, inits[sub], (0.0, 10.0), p)
    pstsol = solve(pstprob, Tsit5())
    push!(ab_sols,pstsol(ab_times[sub])[1:72,:])
