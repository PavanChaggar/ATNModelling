...
pst = deserialize(projectdir("output/chains/population-scaled-atn/pst-samples-$(tracer)-1x1000.jls"));
summarize(pst)
meanpst = mean(pst)
...
begin
    ...
    mean_ab =  vec(mean(reduce(hcat, [d[:, end] for d in ab_conc]), dims=2))
    mean_tau =  vec(mean(reduce(hcat, [d[:, end] for d in tau_conc]), dims=2))
    ...
    mean_ab_diff = vec(mean(reduce(hcat, get_diff.(ab_conc)), dims=2))
    mean_tau_diff = vec(mean(reduce(hcat, get_diff.(tau_conc)), dims=2))
...
save(projectdir("output/plots/inference-results/pst-pred-$(tracer)-scaled.pdf"),f)
