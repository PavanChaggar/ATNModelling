...
_vi = part .+ (β .* (ui .- u0))
inits = [conc.(ab_inits, u0, ui); conc.(tau_inits, v0, _vi); atr_inits]

tspan = (0.0,30.0)

func = make_scaled_atn_model(ui .- u0, part .- v0, L)
sol = simulate(func, inits, tspan, params; saveat=0.1)
d1 = [sol(t, Val{3}) for t in 0:0.1:30]
d = reduce(hcat, [d[73:144] for d in d1])
function atn(D, x, p, t; ui=ui.-u0, part = part.-v0, L=L)
        u = @view x[1:72]
        v = @view x[73:144]
        a = @view x[145:216]
        ...
        D[1:72] .= α_a .* ui .* u .* (1 .- u)
        D[73:144] .= -ρ_t * L * v .+ α_t .* Δ .* v .* ((δ./Δ) .- v)
        D[145:216] .= η .* v .* ( 1 .- a )
end
sol = solve(ODEProblem(atn, inits, (0., 30.0), params), Tsit5(), reltol=1e-9, abstol=1e-9)
fig = plot(sol, color=:grey)
plot!(sol, idxs=72+65, color=:red)
fig
_d1 = [sol(t, Val{1}) for t in 0:0.1:30]
d1 = reduce(hcat, [d[73:144] for d in _d1])
_d2 = [sol(t, Val{2}) for t in 0:0.1:30]
d2 = reduce(hcat, [d[73:144] for d in _d2])
_d3 = [sol(t, Val{3}) for t in 0:0.1:30]
d3 = reduce(hcat, [d[73:144] for d in _d3])

begin
    fig = Figure()
    ax = Axis(fig[1,1])
    plot!(sol, idxs=72 + 65, color=:grey)
    lines!(0:0.1:30, d1[65, :], color=:blue)
    lines!(0:0.1:30, d2[65, :], color=:blue)
    lines!(0:0.1:30, d3[65, :], color=:blue)
    fig
end
