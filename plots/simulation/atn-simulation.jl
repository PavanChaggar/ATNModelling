...
begin
    ...
    Colorbar(gt2[2, 1:2], limits = (minimum(ui), maximum(ui)), colormap = cmap,
    ...
    cb = Colorbar(g1[1:2, 8], limits = (minimum(u0), 1.3), colormap = abcmap,
            vertical = true, labelsize=20, flipaxis=true, ticks=collect(0.6:0.2:1.4),
            ticksize=18, ticklabelsize=20, labelpadding=10)
    cb.alignmode = Mixed(right = 0 )
    cb = Colorbar(g2[1:2, 8], limits = (minimum(v0), 3.5), colormap = taucmap,
    ...
    GLMakie.ylims!(ax, 0.5-0.05, 1.4+0.05)
