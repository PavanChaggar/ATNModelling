...
ab_data_df = filter(x -> x.ab_status == 1 && x.CL_fnc_ber_com_composite < 70, data_df)
...
amyloid_production = mean([meanpst["α_a[$i]", :mean] for i in 1:48])
tau_transport = mean([meanpst["ρ_t[$i]", :mean] for i in 1:48])
tau_production = mean([meanpst["α_t[$i]", :mean] for i in 1:48])
coupling = meanpst["β", :mean]
atrophy = mean([meanpst["η[$i]", :mean] for i in 1:48])
...
ab_threshold = readdlm(projectdir("output/analysis-derivatives/colocalisation/thresholds/ab-thresholds-bf.csv"))
tau_threshold = readdlm(projectdir("output/analysis-derivatives/colocalisation/thresholds/tau-thresholds-bf.csv"))
begin
...
save(projectdir("output/plots/colocalisation/colocalisation-ab-tau-thresholds-random-bf.jpeg"), f)
...
save(projectdir("output/plots/colocalisation/colocalisation-05-tau-thresholds-random-bf.jpeg"), f)
...
ax = Axis3(g3[1,1], aspect = :data, azimuth = 1.0pi, elevation=0.0pi,  protrusions=(0.0,0.0,0.0,0.0))
hidedecorations!(ax); hidespines!(ax)
plot_roi(sort(right_df, :tau_time).DKTID, reverse(collect(range(0, 1, 36))), cmap)