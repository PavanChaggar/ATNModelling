...
ab_data_df = filter(x -> x.ab_status == 1 
            && x.CL_fnc_ber_com_composite < 90, data_df)
...
mean_ab_init = _mean_ab_init_sym
...
# amyloid_production = 0.17 /12
# tau_transport = 0.04 /12
# tau_production = 0.056 /12
# coupling = meanpst["β", :mean]
# atrophy = 0.09 /12
amyloid_production = 0.18 / 12
tau_transport = 0.07 / 12
...
Δ = part .+ (coupling .* (fmm_ui .- fmm_u0))
Lh = inv(diagm(vi .- v0)) * L * diagm(vi .- v0)

atn_pkpd = make_scaled_atn_pkpd_model(fmm_ui[1:36] .- fmm_u0[1:36], part[1:36] .- v0[1:36], L[1:36, 1:36], Ld, m, 28)

sol = simulate(atn_pkpd, [mean_ab_init; mean_tau_init; vol_init; zeros(36); mean_ab_init], 
...
begin
    ...
    lines!(ax2, sol.t, tausol[31, :], linewidth=5)
    lines!(ax1, sol.t, absol[31, :], linewidth=5)
    hlines!(ax2, tau_threshold[31])
    hlines!(ax2, tau_threshold[31])
...
# drug_effect = 0.0
# drug_clearance = 0.1
# noab_sol = simulate(atn_pkpd, [mean_ab_init; mean_tau_init; vol_init; zeros(36); mean_ab_init], 
...
for (i, t) in enumerate(int_ts)
    atn_pkpd = make_scaled_atn_pkpd_model(fmm_ui[1:36] .- fmm_u0[1:36], part[1:36] .- v0[1:36], L[1:36, 1:36], Ld, m, t)
...
begin
    ...
    sc = scatter!(tau_end, atr_end, color=[get(cmap, c) for c in cls])
    ...
    vlines!(sol.t[coloc_t[1][2]], linestyle=:dash, color=:black, linewidth=2.5, label="0.5")
    vlines!(tau_t, color=:black, linewidth=2.5, label="0.79")
    
    scatter!(ax, _ts, atr_diffs, label="Atr", color=[get(cmap, c) for c in cls], marker=:utriangle, markersize=12)
    scatter!(ax, _ts, tau_diffs, label="Tau", color=[get(cmap, c) for c in cls], markersize=12)
    ...
    labels = ["t0 = 0", "t0 = 48","t0 = 120","t0 = 240","Placebo"]
    solidx = [1, 5, 11, 21, 31]
...
save(projectdir("output/plots/pkpd/coloc-pkpd-360-tau-time-bf.jpeg"), fig)
...
for (i, t) in enumerate(t0s)
    atn_pkpd = make_scaled_atn_pkpd_model(fmm_ui[1:36] .- fmm_u0[1:36], part[1:36] .- v0[1:36], Lh, Ld, m, t)

    drug_concentration = 100.
    drug_transport = 0.5 / 12
    drug_effect = 0.075 / 12
    drug_clearance = 0.5 / 12
    _sol = simulate(atn_pkpd, [mean_ab_init; mean_tau_init; vol_init; zeros(36)], 
                    (0.0, 360.0), [amyloid_production, tau_transport, tau_production, 
                                            coupling, atrophy, 
                                            drug_transport, drug_effect, 
                                            drug_concentration, drug_clearance]; 
...
#         atn_pkpd = make_scaled_atn_pkpd_model(fmm_ui[1:36] .- fmm_u0[1:36], part[1:36], Lh, Ld, m, t)
...
#         drug_effect = 0.0
#         drug_clearance = 0.1
#         sol = simulate(atn_pkpd, [mean_ab_init; mean_tau_init; vol_init; zeros(36)], 
...
#     fmm_u0, fmm_ui = load_ab_params(tracer=tracer)
#     fmm_data_df = filter(x -> x.qc_flag==2 && x.TRACER == tracer && x.RID ∈ IDS, _ab_data_df)
#     fmm_data = ADNIDataset(fmm_data_df, dktnames; min_scans=1, reference_region="COMPOSITE_REF")

#     fmm_suvr = calc_suvr.(fmm_data)
#     normalise!(fmm_suvr, fmm_u0, fmm_ui)
#     fmm_conc = map(x -> conc.(x, fmm_u0, fmm_ui), fmm_suvr)
#     fmm_inits = [d[:,1] for d in fmm_conc]
#     mean_ab_inits = mean(fmm_inits)[1:36]
#     println("ab concentration = $(mean_ab_inits[29])")

#     fmm_tau_suvr = calc_suvr.(tau_data)
#     normalise!(fmm_tau_suvr, v0, vi)
#     fmm_tau_conc = map(x -> conc.(x, v0, vi), fmm_tau_suvr)
#     fmm_tau_inits = [d[:,1] for d in fmm_tau_conc]
#     mean_tau_inits = mean(fmm_tau_inits)[1:36]
#     println("tau concentration = $(mean_tau_inits[29])")

#     vol_inits = zeros(36)

#     atn_pkpd = make_scaled_atn_pkpd_model(fmm_ui[1:36] .- fmm_u0[1:36], part[1:36], Lh, Ld, m)
...
#     sol = simulate(atn_pkpd, [mean_ab_inits; mean_tau_inits; vol_inits; zeros(36)], 
